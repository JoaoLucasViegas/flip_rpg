class_name PlayerGridMovement extends Player
...
# Disclaimer 1/7/2026 -> Updating models
# Player class was created so it ease up on casting which model you're using.
# Also, take notice that there's variables that are being used here but doesn't
# appear; So check the parent-class to see the "basics" of what makes a player, a player.
#
# Also take care with "hardcoded" values. There's elements like "AnimatedSprite2D" that might
# bug if you rename it or shift it. Changing it on 1 scene might get you in trouble on another.
# So if you really have to rename it (Why not?! But why?), maybe use the _ready() function
# to adapt this bit of code or else you will need to change every scene that you "broke" by just
